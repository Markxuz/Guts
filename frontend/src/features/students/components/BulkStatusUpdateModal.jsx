      ...
      <div className="my-4 w-full max-w-lg rounded-2xl bg-white shadow-2xl sm:my-0">
        <div className="flex items-center justify-between rounded-t-2xl bg-[#800000] px-5 py-4 text-white">
          <div>
            ...
            <p className="text-xs text-white/80">Apply one status to {selectedCount} selected student{selectedCount === 1 ? "" : "s"}</p>
          </div>
        ...
        <form onSubmit={onSubmit} className="px-5 py-4">
          <div className="mb-4">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3" role="radiogroup" aria-label="Bulk enrollment status options">
              {STATUS_OPTIONS.map((item) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => onChange((current) => ({ ...current, enrollmentStatus: item.value }))}
